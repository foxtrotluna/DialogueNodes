...
signal dialogue_processed(speaker : Variant, listener : Variant, dialogue : String, options : Array[String])
...
func _process_dialogue(dict : Dictionary):
	...
	elif dict.listener is int and characters.size() > 0 and dict.listener < characters.size():
		listener = characters[dict.listener]

	var speakerEmotion = dict.speaker_emotion
	var listenerEmotion = dict.listener_emotion
	...
	dialogue_processed.emit(speaker, speakerEmotion, listener, listenerEmotion, dialogue_text, option_texts)
